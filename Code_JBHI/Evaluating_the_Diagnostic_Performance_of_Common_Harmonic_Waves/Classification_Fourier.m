function [Objectnums_reduce,auc,aupr]=Classification_Fourier(CN,LMCI,path,select_CN_LMCI_same,select)
%% Graph Laplacian
filename=(path);
temp=struct2cell(load(filename));
common_arithmetric=temp{1,1};

CN_Laplacian=cell(2,1);
LMCI_Laplacian=cell(2,1);
...
if Objectnums_less==Objectnums_CN
    for i=1:Objectnums_less
        CN_Laplacian{i,1}=common_arithmetric'*CN(:,i);
        total_data{i,1}=CN_Laplacian{i,1};
        LMCI_Laplacian{i,1}=common_arithmetric'*LMCI(:,select_CN_LMCI_same(i));
        total_data{i+Objectnums_less,1}=LMCI_Laplacian{i,1};
    end
else
    for i=1:Objectnums_less
        CN_Laplacian{i,1}=common_arithmetric'*CN(:,select_CN_LMCI_same(i));
        total_data{i,1}=CN_Laplacian{i,1};
        LMCI_Laplacian{i,1}=common_arithmetric'*LMCI(:,i);
        total_data{i+Objectnums_less,1}=LMCI_Laplacian{i,1};
    end
end
...
% %     mm=mm+1;
% % end
% % test_data=zeros(60,1);
% % test_label=zeros(1,1);
% % train_data=zeros(60,1);
...
[auc,aupr]=Linear_SVM(data_double',label);